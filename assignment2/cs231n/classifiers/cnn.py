...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        pad = (filter_size - 1) // 2
        H_out = 1 + (1 + (1 + (input_dim[1] + 2*pad - filter_size) // 1) - 2) // 2 
        W_out = 1 + (1 + (1 + (input_dim[2] + 2*pad - filter_size) // 1) - 2) // 2 
    ...
    def loss(self, X, y=None):
        ...
        dx, dw, db = affine_backward(scores, affine_cache)
        grads['W3'] = dw + self.reg * affine_cache[1]
        grads['b3'] = db

        dx, dw, db = affine_relu_backward(dx, affine_relu_cache)
        grads['W2'] = dw + self.reg * affine_relu_cache[0][1]
        grads['b2'] = db

        dx, dw, db = conv_relu_pool_backward(dx, conv_relu_pool_cache)
        grads['W1'] = dw + self.reg * conv_relu_pool_cache[0][1]
        grads['b1'] = db
